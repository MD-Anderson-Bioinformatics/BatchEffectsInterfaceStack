...
function getBEIPropertiesCommon(theSelf)
{
	// Do an async Ajax call to BEI properties to get defaults.
	...
	{ 
		...
		{
			//console.log("BEIproperties: success");
			// == not ===
			theSelf.serverTitle((null===theJson.serverTitle) ? theSelf.serverTitle() : theJson.serverTitle);
			theSelf.propAllowLogin(
				// use == instead of ===
				(theJson.allowLogin === null) ? theSelf.propAllowLogin() : theJson.allowLogin
			);
		},
		error: function(jqXHR, textStatus, errorThrown)
		{
			console.log("BEIproperties: " + textStatus + " and " + errorThrown);
			alert("BEIproperties: " + textStatus + " and " + errorThrown);
		}
	});
}
...
function setupKnockoutValidation()
{
	...
	// 
	// I changed that so that the ‘beierror’ class and error message would  still be applied 
	...
	// ‘beirror’ class isn’t applied. It’s even more confusing at the GUI level because
	...
	return(
	{
		...
		errorMessageClass: 'beierror',
		errorElementClass: 'beierror',
		errorClass: 'beierror',
		errorsAsTitle: true,
...
function getLoggedInUser(theSelf)
{
	...
	{ 
		...
		success: function(theJson)
		{
			theSelf.currentUserName((null===theJson.userName) ? "" : theJson.userName);
			theSelf.availableUsers((null===theJson.availableUsers) ? theSelf.availableUsers() : theJson.availableUsers);
			theSelf.availableRoles((null===theJson.availableRoles) ? theSelf.availableRoles() : theJson.availableRoles);
		},
		error: function(jqXHR, textStatus, errorThrown)
		{
