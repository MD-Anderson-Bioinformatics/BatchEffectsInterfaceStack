 ...
 * Description:
 * -----------------------------------------------------------------------------
 * A class to represent the row and column structure of a matrix data file. Makes
 * a first pass through the file and creates an index of the columns and rows. A
 * column index is the columns numerical position with respect to the original column
 * header order. A row index is the byte offset at which that row begins in the file.
 * Rows/Columns can then be sorted/filtered by the row/column header. The write method
 * then writes out the matrix, preserving any sorting/filtering done with the instance.
 * 
 * The column/row properties are ArrayLists of Header instance, which will represent
 * ordering and allow sorting. The columnSet/rowSet properties are HashSets which represent
 * the register of current entries and allow filtering.
 * 
 * Notes:
 * -----------------------------------------------------------------------------
 *  - Using the custom BufferedRandomAccessFile as a solution to getting a persistent
 *    accurate byte-offset. Also gives performance improvement over default RandomAccessFile.
 *  - The meat and potatoes of this class are in the methods initRows, initColumns, and write.
 *    The other methods and data structure manipulations are fairly straight forward.
 */
...
public final class Matrix {
    ...
    /**
     * Pseudo public constructor. Instantiate Matrix from single path argument, assuming
     * defaults for all other parameters. In all other use cases, use Builder.
     * @param path
     * @return 
     ...
     * instance contains the row label, and the byte offset for the beginning
     * of the row.
     * @throws java.io.FileNotFoundException
     * @throws java.io.IOException
     */
    ...
    /**
     * Sort an ArrayList<Header>, either columns or rows.
     * TODO: Possibly allow sorting order as argument.
     * 
     * @param headers
     */
     ...
     * Iterates on the ArrayList row/columns instances, but checks HashSet rowSet/columnSet
     * instances to see if an entry was filtered out.
     * 
     * @param outPath
     * @throws java.io.FileNotFoundException
     * @throws java.io.IOException
    ...
    /**
     * Get the columns.
     * @return <ArrayList<Header>>: The columns. 
     */
    ...
    /**
     * Get the rows.
     * @return <ArrayList<Header>>: The rows. 
     */
    ...
    /**
     * Get the columnSet.
     * @return <HashSet<String>>: The columnSet. 
     */
    ...
    /**
     * Get the rowSet.
     * @return <HashSet<String>>: The rowSet. 
     */
    ...
    /**
     * Add a row. All row values for the added row will be set to the
     * Matrix's standIn instance property.
     * 
     ...
     * @param <String> Line from a readLine() execution.
     ...
     * headers in m1 that aren't also in m2. Specific to an axis, either row or column.
     * 
     * @param matrix <Matrix>: The matrix to be filtered.
     * @param filterSet <HashSet>: The filter set.
     * @param axis <int>: The axis, 0 (row) or 1 (column)
     */
    ...
    /**
     * Write a current Matrix instance to the same path it was built form. Requires
     * re-instantiating matrix upon completion.
     */
